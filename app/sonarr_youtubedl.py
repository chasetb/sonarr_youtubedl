...
class SonarrYTDL:
        ...
        for ser in series[:]:
            for wnt in self.series:
                if wnt["title"] == ser["title"]:
                    ...
                    # Read exclude_words and exclude_patterns
                    if "exclude_words" in wnt:
                        ser["exclude_words"] = wnt["exclude_words"]
    ...
    def ytdl_eps_search_opts(
        self, regextitle, playlistreverse, cookies=None, exclude_words=None, exclude_patterns=None
    ):
        ytdlopts = {
            "ignoreerrors": True,
        ...
        if exclude_words:
            # Escape words to handle special regex characters
            escaped_words = [re.escape(word) for word in exclude_words]
            # Build the reject pattern
            reject_pattern = r"(?i)({})".format("|".join(escaped_words))
            ytdlopts["rejecttitle"] = reject_pattern

        if exclude_patterns:
            # Combine patterns
            reject_pattern = r"(?i)({})".format("|".join(exclude_patterns))
            ytdlopts["rejecttitle"] = reject_pattern
            ...
            for _s, ser in enumerate(series):
                ...
                for e, eps in enumerate(episodes):
                    if ser["id"] == eps["seriesId"]:
                        ...
                        # Retrieve exclude words/patterns
                        exclude_words = ser.get("exclude_words", None)
                        exclude_patterns = ser.get("exclude_patterns", None)

                        # Build the yt-dlp options with exclude patterns
                        ydleps = self.ytdl_eps_search_opts(
                            upperescape(eps_title),
                            ser["playlistreverse"],
                            cookies,
                        ...
                        if found:
                            ...
                            filename_template = "{series}.S{season_number}E{episode_number:02d}.{episode_title}"
                            video_filename = (
                                filename_template.format(
                                    series=ser_title,
                                    season_number=eps["seasonNumber"],
                                    episode_number=eps_number,
                                    episode_title=eps_title,
                                )
                                + ".%(ext)s"
                            )
                            ...
                            video_output = str(season_folder / video_filename)
                            thumbnail_output = str(season_folder / thumbnail_filename)
